...
import { Loader2, ArrowLeft, Calendar, Shield, MonitorPlay } from 'lucide-react';
...
export default function Event() {
    ...
    useEffect(() => {
        ...
        getEvents().then(data => {
            ...
            if (found) {
                ...
                // Find related: same category, excluding current
    ...
    if (!match) return <div className="h-screen flex items-center justify-center bg-background text-white">Match not found</div>;
                ...
                <div className="flex flex-col lg:flex-row gap-8 lg:items-start justify-between border-b border-white/5 pb-10">
                    <div className="flex-1">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            {isLive ? (
                                <span className="bg-live text-white px-3 py-1 rounded text-xs font-bold animate-pulse shadow-lg shadow-red-900/40">LIVE NOW</span>
                            ) : (
                                <span className="bg-yellow-600/80 text-white px-3 py-1 rounded text-xs font-bold">UPCOMING</span>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-4 text-gray-400 text-lg">
                            ...
                            <div className="text-white font-medium">{format(date, 'h:mm a')}</div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-3 min-w-[320px] bg-[#111] p-5 rounded-xl border border-white/5">
                        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                            <MonitorPlay className="w-4 h-4" /> Available Streams
                        </h3>
                        <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                            ...
                            {match.substreams?.map((sub, idx) => (
                                ...
                                >
                                    <span>{sub.name || `Alternative ${idx + 2}`}</span>
